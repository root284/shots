...
export default function StoryboardTool() {
  ...
  const buildStage1Prompt = () =>
    `당신은 애니메이션 연출가입니다. 아래 글을 상세한 글 콘티로 변환하세요.
...
【전체 톤】
분위기, 색감 방향, 음악 느낌 (예: 고독하고 서정적 / 파스텔 블루-그레이 / 피아노 앰비언트)

【감정선】
씬 전체 감정 흐름 한 줄 (예: 상실 → 체념 → 작은 해방)

【연출 포인트】
· (3~5개, 연출가 노트 형식으로. 카메라 시선, 인물 연기, 감정 표현 방법 등)

【컷 구상】
(${Math.round(seconds / 1.5)}~${Math.round(seconds / 1.2)}컷 목표, 전체 약 ${seconds}초)
1. [샷 사이즈/앵글] | 화면: ... | 연기: ... | 감정: ... | ~Xs
2. ...

※ 내면 묘사("불안했다" 등)는 반드시 표정·시선·소도구·카메라워크로 외화할 것.
※ 설명이나 메타 코멘트 없이 위 형식만 출력.
